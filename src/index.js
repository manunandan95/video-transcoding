...
const convertmp4Object = object =>
  ffmpeg(['-i', dlPath(object.movKey), '-vcodec', 'h264','-acodec','mp2','-y', dlPath(object.mp4Key)])
    .then(() => object);

const convertwebmObject = object =>
  ffmpeg(['-i', dlPath(object.movKey), '-c', 'libvpx','-pix_fmt','yuva420p','-b','5M','-maxrate','5M','-bufsize','1M','-auto-alt-ref','0','-y', dlPath(object.webmKey)])
    .then(() => object);
...
class S3Object {
  constructor(bucket, key) {
    ...
    this.webmKey = this.movKey.replace(/\.mov$/, '.webm');
  }
...
class MovToMp4 {
  ...
  processObjects(objects) {
    return Promise.all(objects.map(object =>
      State.info('Starting download', { object, path: dlPath(object.movKey) })(object)
        ...
        .then(convertmp4Object)
        .then(convertwebmObject)
        .then(State.info('Uploading converted object'))
        .then(this.uploadConvertedmp4Object.bind(this))
        .then(this.uploadConvertedwebmObject.bind(this))
 ...
 uploadConvertedmp4Object(object) {
    return this.s3.putObject({
      Bucket: 'output24gbucket',
      Key: 'mp4/' +  object.mp4Key,
      Body: createReadStream(dlPath(object.mp4Key)),
      ContentType: 'video/mp4'
    }).promise().then(() => object);
  }
  uploadConvertedwebmObject(object) {
    return this.s3.putObject({
      Bucket: 'output24gbucket',
      Key: 'webm/' +  object.webmKey,
      Body: createReadStream(dlPath(object.webmKey)),
